 ...
 */
export const login = (credentials: Credentials): Effect.Effect<string, Error, never> =>
  Effect.gen(function* () {
    ...
    const response = yield* Effect.tryPromise({
      ...
      catch: (e) => (e instanceof Error ? e : new Error('Login request failed'))
    })
    ...
    if (!cookieHeader) {
      return yield* Effect.fail(new Error('Missing Set-Cookie in login response'))
    }
